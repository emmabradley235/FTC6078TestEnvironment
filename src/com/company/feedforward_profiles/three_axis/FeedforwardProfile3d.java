...
public abstract class FeedforwardProfile3d {
    ...
    protected FeedforwardProfile3d(double profileDuration, ProfileEndBehavior endBehavior){ // constructor (called using super() by child classes)
        this.profileDuration = profileDuration;
        this.endBehavior = endBehavior;
    }


    public final Pose2d getPrimaryTarget(double timestep){ // the method called to get a profile's primary (think position) target, the method it calls (getProfileTarget) will change depending on subclass, but still always handles end behaviour before passing in
        return getPrimaryProfileTarget( handleEndBehavior(timestep) );
    }
    public final Pose2d getSecondaryTarget(double timestep) { // the method called to get a profile's primary (think velocity) target
        return getSecondaryProfileTarget( handleEndBehavior(timestep) );
    }
    public final Pose2d getTertiaryTarget(double timestep){ // the method called to get a profile's primary (think acceleration) target
        return getTertiaryProfileTarget( handleEndBehavior(timestep) );
    }
    protected Pose2d getPrimaryProfileTarget(double timestep){ // to be overriden by subclasses
        return new Pose2d();
    }
    protected Pose2d getSecondaryProfileTarget(double timestep){ // to be overriden by subclasses
        return new Pose2d();
    }
    protected Pose2d getTertiaryProfileTarget(double timestep){ // to be overriden by subclasses
        return new Pose2d();
    }
