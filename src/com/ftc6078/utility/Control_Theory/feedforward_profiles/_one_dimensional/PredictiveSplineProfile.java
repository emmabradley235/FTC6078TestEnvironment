...
public class PredictiveSplineProfile extends FeedforwardProfile {
    PredictiveSplineInterpolator predictiveSplineInterp; // the thing that interpolates between the setpoints
    ...
    public PredictiveSplineProfile(ArrayList<TimestampedValue> timestampedSetpoints, double circleRadius, double primaryK, double secondaryK, double tertiaryK, ProfileEndBehavior endBehavior){
        ...
        this.predictiveSplineInterp = new PredictiveSplineInterpolator( timestampedSetpoints, circleRadius ); // setup the multislope linear interpolator

        this.K1 = primaryK;
        this.K2 = secondaryK;
        this.K3 = tertiaryK;
    }
    public PredictiveSplineProfile(ArrayList<TimestampedValue> timestampedSetpoints, double circleRadius, double primaryK, double secondaryK, double tertairyK){
    ...
    public PredictiveSplineProfile(ArrayList<TimestampedValue> timestampedSetpoints, double circleRadius, ProfileEndBehavior endBehavior){
        this(timestampedSetpoints, circleRadius,1.0, 1.0, 1.0, endBehavior);  // if no value passed, use a gain of 1 to not modify values
    }
    public PredictiveSplineProfile(ArrayList<TimestampedValue> timestampedSetpoints, double circleRadius){
    ...
    public double getPrimaryProfileTarget(double timestep){ // think position
        return K1 * predictiveSplineInterp.interpolate( timestep ); // then use that to figure out what our current point should be between the two points
    }

    @Override
    public double getSecondaryProfileTarget(double timestep){ // think velocity
        return K2 * predictiveSplineInterp.getSubinterpolatorForTimestep( timestep ).getSlopeAt(timestep);
    }

    @Override
    public double getTertiaryProfileTarget(double timestep){ // think velocity
        return K3 * predictiveSplineInterp.getSubinterpolatorForTimestep( timestep ).getSlopeOfSlopeAt( timestep );
    }
}
